...
function Strategy(options, verify) {
  ...
  this._usernameHeader = options.usernameHeader.toLowerCase() || 'x-username';
  this._tokenHeader    = options.tokenHeader.toLowerCase()    || 'x-token';
  
  this._usernameField = options.usernameField.toLowerCase() || 'username';
  this._tokenField    = options.tokenField.toLowerCase()    || 'token';
...
Strategy.prototype.authenticate = function(req, options) {
  options = options || {};
  var self = this;
  var username = req.headers[this._usernameHeader] || lookup(req.body, this._usernameField);
  var token    = req.headers[this._tokenHeader] || lookup(req.body, this._tokenField);
